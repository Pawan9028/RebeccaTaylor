...
public class ProductATCFlowPage  {
	...
	public ProductATCFlowPage(WebDriver driver) {
		this.driver = driver;
	...
	@FindBy(xpath = "//details[@class='search_details search_details--popup header--icon-border']") 
	private WebElement searchIcon;
	@FindBy(xpath = "//input[@id='Search-In-Modal-Menu-Plain']")private WebElement searchBox;
	@FindBy(xpath = "//div[@class='swiper-wrapper']")private List<WebElement>listOfSearchProducts;
	@FindBy(id = "product-add-to-cart")private WebElement addToCartBtn;
	@FindBy(xpath = "//div[@class='header--account header--icon-border h-100 d-flex flex-align-center p-relative']")
	...
	@FindBy(xpath = "//button[@class='button button-1 button-checkout']")private WebElement checkoutBtn;
	@FindBy(xpath = "//div[@class='_1fragem1y _1fragemlj']/div/span/a")private List<WebElement>listOfCheckoutPageLogo;
	@FindBy(xpath = "//div[@class='account-action']/a[contains(text(),'Logout')]")private WebElement logOutBtn;
	@FindBy(xpath = "//div[@class='form-actions']/input[@class='button button-2']")private List<WebElement> listOfSubmitBtn;
	@FindBy(xpath = "//div[@class='header-logo header-item header-item--side d-flex flex-align-center d-flex fs-0 fg-0 pt-2 pb-5']") private WebElement logo;
	...
	public void logInMethod() throws InterruptedException {
		pop_up_Handle();
		account_tab.click();
		emailID.sendKeys("pavan@aristasystems.in");
		pass.sendKeys("Pawan@12345");
		Thread.sleep(1000);
		listOfSubmitBtn.get(1).click();	
	}
	...
	public void productSearchMethod() throws InterruptedException {
		...
		Thread.sleep(100);
		listOfSearchProducts.get(1).click();
	}
	...
	//// User general flow of shopping///
	public void verifyUserFlowOfShopping() throws InterruptedException {
		pop_up_Handle();
		//logInMethod();
		productSearchMethod();
		VerifyPageScrollable();
		productATCmethod();
		checkoutTest();
		//logOutMethod();
		System.out.println("General user shopping flow is tested by searching the 'Dress' products and added to the cart.");
	}
}
